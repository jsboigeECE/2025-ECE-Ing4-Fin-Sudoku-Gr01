...
namespace Sudoku.recuit_simule
{
    public class recuit_temperature_v2: ISudokuSolver
    {
        ...
        {
            // Test de plusieurs températures et sauvegarde des résultats
            TesterPlusieursTemperatures(sudokuToSolve);
        ...
        private void TesterPlusieursTemperatures(SudokuGrid sudokuToSolve)
        {
            string filePath = "resultats_recuit_temperature2.csv";
            ...
            using (StreamWriter writer = new StreamWriter(filePath))
            {
                ...
                for (double temperature = 0.0; temperature <= 20.0; temperature += 0.2)
                {
                    ...
                    var (_, coutFinal) = RecuitSimuleSudoku(grilleBase, temperature, 0.99, 200000);
                    ...
                    writer.WriteLine($"{temperature.ToString(CultureInfo.GetCultureInfo("fr-FR"))};{tempsExecution};{coutFinal}");

                    writer.Flush(); // Assurer l'écriture immédiate dans le fichier

                    // Afficher les résultats dans la console
                    Console.WriteLine($"Température : {temperature}, Temps : {tempsExecution}ms, Coût : {coutFinal}");
                }
            }
